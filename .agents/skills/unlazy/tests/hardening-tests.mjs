...
function gateRun(s, args, options = {}) {
  const action = args.some((arg) =>
    ["--status", "--claim", "--release", "--list-scopes", "--log", "--bind", "--help"].includes(
      arg,
    ),
  );
  const actual =
    options.approve !== false && !action && !args.includes("--approve")
      ? ["--approve", ...args]
      : args;
...
const has = (text, value, label = "output") =>
  assert(text.includes(value), label + " missing " + JSON.stringify(value) + "\n" + text);
const lacks = (text, value, label = "output") =>
  assert(
    !text.includes(value),
    label + " unexpectedly includes " + JSON.stringify(value) + "\n" + text,
  );
...
test("approval: status is read-only and an unapproved CHECK is printed but not run", async () => {
  ...
  try {
    s.write(
      "check.mjs",
      "import { writeFileSync } from 'node:fs'; writeFileSync('ran.txt','yes'); console.log('OK');\n",
    );
    s.write("GATES.md", gate("G1", "approval", "node check.mjs", "OK"));
    const before = s.read("GATES.md");
    const status = await gateRun(s, ["--status"], {
      approve: false,
      env: { UNLAZY_SHELL: "definitely-missing-shell" },
    });
    ...
    assert(
      !s.path("ran.txt") || !(await fileExists(s.path("ran.txt"))),
      "unapproved CHECK executed",
    );
    s.write(
      "GATES.md",
    ...
    assert(
      unverified.code === 1,
      "unapproved reverify must not certify existing evidence\n" + unverified.out,
    );
...
test("approval: token binds the full oracle and changing CWD invalidates it", async () => {
  ...
  try {
    ...
    for (const key of [
      ...
      assert(
        Object.prototype.hasOwnProperty.call(token.oracle, key),
        "approval oracle missing " + key,
      );
    }
...
test("approval: storage failures use the infrastructure exit code", async () => {
  ...
  try {
    ...
    assert(
      result.code === 2,
      "approval storage failure returned " + result.code + "\n" + result.out,
    );
...
test("approval: a lexical outside alias cannot resolve back into the repository", async () => {
  ...
  try {
    ...
    assert(
      !existsSync(s.path("ran.txt")),
      "a repo-controlled approval alias authorized CHECK execution",
    );
  } finally {
    s.cleanup();
  }
...
test("parser: malformed ledgers are usage errors in checker and blocking in hook", async () => {
  ...
  for (const [name, text, expected] of cases) {
    ...
    try {
      ...
      const hook = await run(STOP_HOOK, [], {
        cwd: s.dir,
        stdin: JSON.stringify({ cwd: s.dir, session_id: name }),
      });
...
test("jobs: rolling concurrency is opt-in and output stays in gate order", async () => {
  ...
  try {
    for (let index = 1; index <= 3; index++) {
      s.write(
        ...
        "import { appendFileSync } from 'node:fs';\n" +
          "appendFileSync('order.log','start" +
          index +
          "\\n');\n" +
          "setTimeout(()=>{appendFileSync('order.log','end" +
          index +
          "\\n'); console.log('OK" +
          index +
          "');},250);\n",
    ...
    s.write(
      "GATES.md",
      gate("G1", "one", "node g1.mjs", "OK1") +
        gate("G2", "two", "node g2.mjs", "OK2") +
        gate("G3", "three", "node g3.mjs", "OK3"),
    );
    ...
    assert(
      result.out.indexOf("PASS GATES:G1") < result.out.indexOf("PASS GATES:G2") &&
        result.out.indexOf("PASS GATES:G2") < result.out.indexOf("PASS GATES:G3"),
      "transcript order was nondeterministic\n" + result.out,
    );
  } finally {
    s.cleanup();
  }
...
test("writeback: a result cannot certify a gate whose oracle changed in flight", async () => {
  ...
  try {
    s.write(
      "slow.mjs",
      "import { writeFileSync } from 'node:fs'; writeFileSync('started.txt','yes'); setTimeout(()=>console.log('OLD'),800);\n",
    );
...
test("reverify: a stale in-flight result cannot leave old evidence falsely green", async () => {
  ...
  try {
    ...
    assert(
      s.read("GATES.md").includes("EXPECT: NEW\n  EVIDENCE: old evidence"),
      "newer ledger was clobbered",
    );
  } finally {
    s.cleanup();
  }
...
test("CLI: numeric bounds, incompatible modes, and file types fail with exit 2", async () => {
  ...
  try {
    ...
    for (const args of cases) {
      ...
      assert(
        result.code === 2,
        args.join(" ") + " expected 2, got " + result.code + "\n" + result.out,
      );
    }
  } finally {
    s.cleanup();
...
test("leases: unsafe declarations, unknown leaves, and wildcard witnesses fail closed", async () => {
  ...
  try {
    ...
    const first = await gateRun(s, ["--scope", "a", "--leaf", "leaf-a", "--claim"], {
      approve: false,
    });
    assert(first.code === 0, first.out);
    const overlap = await gateRun(s, ["--scope", "b", "--leaf", "leaf-b", "--claim"], {
      approve: false,
    });
    assert(overlap.code === 3, overlap.out);
    has(overlap.out, "CONFLICT src/ab*.js overlaps src/a*.js");
    const unknown = await gateRun(s, ["--scope", "a", "--leaf", "missing", "--claim"], {
      approve: false,
    });
    ...
    s.write(
      ".unlazy/dot-a/gates/leaf.md",
      "OWNS: src/./same.js\n\n" + gate("G1", "dot a", null, null),
    );
    s.write(
      ".unlazy/dot-b/gates/leaf.md",
      "OWNS: src/same.js\n\n" + gate("G1", "dot b", null, null),
    );
    const dotFirst = await gateRun(s, ["--scope", "dot-a", "--leaf", "leaf", "--claim"], {
      approve: false,
    });
    assert(dotFirst.code === 0, dotFirst.out);
    has(dotFirst.out, "src/same.js");
    const dotOverlap = await gateRun(s, ["--scope", "dot-b", "--leaf", "leaf", "--claim"], {
      approve: false,
    });
    ...
    s.write(
      ".unlazy/prefix-a/gates/leaf.md",
      "OWNS: src/api\n\n" + gate("G1", "prefix a", null, null),
    );
    s.write(
      ".unlazy/prefix-b/gates/leaf.md",
      "OWNS: src/api/**\n\n" + gate("G1", "prefix b", null, null),
    );
    const prefixFirst = await gateRun(s, ["--scope", "prefix-a", "--leaf", "leaf", "--claim"], {
      approve: false,
    });
    assert(prefixFirst.code === 0, prefixFirst.out);
    const prefixOverlap = await gateRun(s, ["--scope", "prefix-b", "--leaf", "leaf", "--claim"], {
      approve: false,
    });
    ...
    const unsafe = await gateRun(s, ["--scope", "c", "--leaf", "leaf-c", "--claim"], {
      approve: false,
    });
...
test("win32 cleanup: inconsistent system directory variables fail closed", async () => {
  ...
  for (const env of mismatches) {
    assert(
      windowsTaskkillPath(env) === null,
      "trusted inconsistent Windows environment: " + JSON.stringify(env),
    );
  }
});
...
test("win32 cleanup: missing trusted system root skips PATH lookup", async () => {
  ...
  assert(
    windowsTaskkillPath({ PATH: "C:\\attacker" }) === null,
    "bare taskkill fallback remained enabled",
  );
...
test("win32 integration: a timed-out shell and nested descendant are both reaped", async () => {
  ...
  try {
    ...
    assert(
      ...
      "invalid captured PIDs: " + pids.join(","),
    );
...
test("execution: timeout settlement is bounded even when a detached descendant keeps pipes", async () => {
  ...
  try {
    ...
    s.write(
      "GATES.md",
      gate("G1", "escaped pipe holder times out", "node escape.mjs", "never printed"),
    );
    const started = Date.now();
    const result = await gateRun(s, ["--timeout", "1"]);
    const elapsed = Date.now() - started;
    ...
    assert(
      elapsed < 7000,
      "checker did not settle independently after cleanup request: " + elapsed + "ms",
    );
...
test("posix integration: an exited shell leader still has its ordinary descendants reaped", async () => {
  ...
  try {
    ...
    s.write(
      "GATES.md",
      gate("G1", "orphaned group member times out", "node exiting-parent.mjs", "never printed"),
    );
    const result = await gateRun(s, ["--timeout", "1"]);
    assert(result.code === 1, result.out);
    has(result.out, "timed out after 1s");
    ...
    assert(
      !existsSync(s.path("ordinary-marker.txt")),
      "ordinary process-group descendant survived cleanup",
    );
  } finally {
    if (Number.isInteger(descendantPid) && descendantPid > 0) {
      try {
