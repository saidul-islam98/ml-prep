...
check("every local resource the skill names exists", () => {
  ...
  const missing = [];
  const named = new Set();
  for (const m of skill.matchAll(/`((?:references|templates|scripts)\/[^`\s]+|SECURITY\.md)`/g))
    named.add(m[1]);
...
check("request reconciliation keeps the focused solo cheap path", () => {
  const skill = read("SKILL.md");
  if (!skill.toLowerCase().includes("re-read the current request"))
    return "SKILL.md lacks final request reread";
  if (!skill.includes("a PLAN table is not required"))
    return "focused solo path was made needlessly orchestrated";
