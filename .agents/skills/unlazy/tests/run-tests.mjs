...
function run(script, args, opts = {}) {
  return new Promise((res) => {
    ...
    const needsApproval =
      script === GATE_CHECK && !opts.noApprove && !args.some((arg) => actions.has(arg));
...
function assertHas(hay, needle, label) {
  assert(
    hay.includes(needle),
    (label || "output") + " missing " + JSON.stringify(needle) + "\n--- got ---\n" + hay,
  );
}
...
test("scope: running one pipeline leaves the other pipeline alone", async () => {
  ...
  try {
    s.write(
      ".unlazy/api/gates/leaf-1.md",
      "# Gates: api1\n\n" + gate("G1", "api", echoOk("API-OK"), "API-OK"),
    );
    s.write(
      ".unlazy/web/gates/leaf-1.md",
...
test("state: abandoned and mixed ledgers stay handoffs in every run mode", async () => {
  for (const args of [["--status"], [], ["--reverify"]]) {
    ...
    try {
      ...
      assert(
        r.code === 1,
        "expected exit 1 for " + JSON.stringify(args) + ", got " + r.code + "\n" + r.out,
      );
      assertHas(r.out, "HANDOFF REQUIRED: 1 abandoned");
      assertLacks(r.out, "ALL MET");
    } finally {
...
test("run: a concurrent edit to another gate in the same file survives", async () => {
  ...
  try {
    ...
    s.write(
      ...
      "# Gates\n\n" +
        "- [ ] G1: slow\n  CHECK: " +
        nodeEval("setTimeout(()=>console.log('SLOW-OK'),2500)") +
        "\n  EXPECT: SLOW-OK\n  EVIDENCE: pending\n\n" +
        "- [ ] G2: manual\n  EVIDENCE: pending\n",
    );
...
test("checks: CWD runs a check in the directory it names", async () => {
  ...
  try {
    ...
    s.write(
      ...
      "# Gates\n\n" +
        "- [ ] G1: in sub\n  CHECK: " +
        nodeEval("console.log(require('fs').readFileSync('marker.txt','utf8'))") +
        "\n  EXPECT: here\n  CWD: sub\n  EVIDENCE: pending\n",
    );
    const r = await run(GATE_CHECK, [], { cwd: s.dir });
    assertHas(r.out, "ALL MET (1 met)");
...
test("leases: overlapping OWNS across pipelines is refused", async () => {
  ...
  try {
    ...
    s.write(
      ".unlazy/web/gates/leaf-1.md",
      "OWNS: src/shared/util.ts\n\n# Gates\n\n" + gate("G1", "b", null, null),
    );
    const a = await run(GATE_CHECK, ["--scope", "api", "--leaf", "leaf-1", "--claim"], {
      cwd: s.dir,
    });
    assertHas(a.out, "CLAIMED 1 path(s) for api/leaf-1");
    const b = await run(GATE_CHECK, ["--scope", "web", "--leaf", "leaf-1", "--claim"], {
      cwd: s.dir,
    });
...
test("leases: disjoint OWNS both succeed, and release frees them", async () => {
  ...
  try {
    ...
    s.write(
      ".unlazy/api/gates/leaf-2.md",
      "OWNS: src/web/**\n\n# Gates\n\n" + gate("G1", "b", null, null),
    );
    const a = await run(GATE_CHECK, ["--scope", "api", "--leaf", "leaf-1", "--claim"], {
      cwd: s.dir,
    });
    const b = await run(GATE_CHECK, ["--scope", "api", "--leaf", "leaf-2", "--claim"], {
      cwd: s.dir,
    });
...
test("hook: does not block on a pipeline this session does not own", async () => {
  ...
  try {
    ...
    const r = await run(STOP_HOOK, ["--scope", "api"], {
      cwd: s.dir,
      stdin: JSON.stringify({ cwd: s.dir }),
    });
...
test("hook: session binding resolves the scope among several", async () => {
  ...
  try {
    ...
    const r = await run(STOP_HOOK, [], {
      cwd: s.dir,
      stdin: JSON.stringify({ cwd: s.dir, session_id: "sess-abc" }),
    });
    assertHas(r.out, '"decision":"block"');
    assertHas(r.out, "[scope web]");
  } finally {
