...
const has = (text, value) =>
  assert(text.includes(value), "missing " + JSON.stringify(value) + "\n" + text);
...
test("leases: 200 simultaneous conflicting claim pairs never both succeed", async () => {
  ...
  try {
    for (let iteration = 0; iteration < 200; iteration++) {
      ...
      assert(
        successes === 1,
        "iteration " + iteration + " had " + successes + " successful claims",
      );
      await Promise.all([
...
test("leases: the same scope and leaf cannot be claimed by two workers", async () => {
  ...
  try {
    ...
    assert(
      Number(left.ok) + Number(right.ok) === 1,
...
test("leases: an explicit release cleans orphaned leases after a scope directory is gone", async () => {
  const s = sandbox();
  try {
    const claimed = await claimLeases(s.dir, {
      scope: "gone",
      leaf: "leaf-gone",
      globs: ["src/gone/**"],
    });
    assert(claimed.ok, "fixture lease was not created");
    const result = await run(GATE_CHECK, ["--scope", "gone", "--leaf", "leaf-gone", "--release"], {
      cwd: s.dir,
    });
    ...
    assert(
      !lockNames.some((name) => name.endsWith(".lease")),
      "orphan lease remained: " + lockNames.join(", "),
    );

    s.write(".unlazy/live/gates/leaf-live.md", "# malformed zero-gate ledger\n");
    const liveClaim = await claimLeases(s.dir, {
      scope: "live",
      leaf: "leaf-live",
      globs: ["src/live/**"],
    });
    assert(liveClaim.ok, "live fixture lease was not created");
    const liveRelease = await run(
      GATE_CHECK,
      ["--scope", "live", "--leaf", "leaf-live", "--release"],
      { cwd: s.dir },
    );
  ...
  try {
    ...
    for (let round = 1; round <= 3; round++) {
      ...
      const results = await Promise.all(
        Array.from({ length: 64 }, () => run(STOP_HOOK, [], { cwd: s.dir, stdin: payload })),
      );
      ...
      assert(
        ...
        "round " +
          ...
          " hook(s) failed open on the state update\n" +
      ...
      assert(
        sessions.length === 1,
        "round " + round + ": expected one session, got " + sessions.length,
      );
      assert(
        sessions[0].blocks === 64,
        "round " + round + ": expected 64 blocks, got " + sessions[0].blocks,
      );
    }
  } finally {
    s.cleanup();
...
test("hook: sessions remain isolated and completion/no-gates clears stale state", async () => {
  ...
  try {
    ...
    for (const id of ["alpha", "beta"]) {
      ...
      await Promise.all(
        Array.from({ length: 3 }, () => run(STOP_HOOK, [], { cwd: s.dir, stdin: payload })),
      );
    }
    let state = JSON.parse(s.read(".unlazy-hook-state.json"));
    assert(
      Object.keys(state.sessions).length === 2,
      "sessions were mixed: " + JSON.stringify(state),
    );
    assert(
      Object.values(state.sessions).every((value) => value.blocks === 3),
      "session counters were not isolated",
    );

    s.write("GATES.md", "- [x] G1: done\n  EVIDENCE: measured\n");
    await run(STOP_HOOK, [], {
      cwd: s.dir,
      stdin: JSON.stringify({ cwd: s.dir, session_id: "alpha" }),
    });
    ...
    await run(STOP_HOOK, [], {
      cwd: s.dir,
      stdin: JSON.stringify({ cwd: s.dir, session_id: "beta" }),
    });
    assert(
      !existsSync(s.path(".unlazy-hook-state.json")),
      "no-gates path did not clear final state",
    );
  } finally {
    s.cleanup();
  }
...
test("status log: an existing symlink is refused without touching its target", async () => {
  ...
  try {
    ...
    const result = await run(
      GATE_CHECK,
      ["--scope", "api", "--log", "attacker-controlled append"],
      { cwd: s.dir },
    );
...
test("installer: uninstall preserves a sibling in the same matcher group and writes a backup", async () => {
  ...
  try {
    ...
    assert(
      s.read(".claude/settings.local.json.unlazy.bak") === original,
...
test("installer: marker substrings do not claim an unrelated stop hook", async () => {
  ...
  try {
      ...
      JSON.stringify(
        {
          hooks: {
            Stop: [
              { hooks: unrelated.map((command) => ({ type: "command", command, timeout: 20 })) },
            ],
          },
        },
        null,
    ...
    const after = JSON.parse(s.read(".claude/settings.local.json"));
    assert(
      after.hooks.Stop[0].hooks.map((hook) => hook.command).join("\n") === unrelated.join("\n"),
...
test("installer: a matching command with broken managed fields is repaired", async () => {
  ...
  try {
    ...
    const after = JSON.parse(s.read(".claude/settings.local.json"));
    const managed = after.hooks.Stop.flatMap((group) => group.hooks).filter(
      (item) => typeof item.command === "string" && item.command.includes("--unlazy"),
    );
