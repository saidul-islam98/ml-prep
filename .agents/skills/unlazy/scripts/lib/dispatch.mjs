...
import { appendStatus, scopeRoot, validateScopeId, withFileLock, writeAtomic } from "./gates.mjs";
...
const safeDiagnostic = (value) =>
  String(value).replace(CONTROL_GLOBAL, " ").replace(/\s+/g, " ").trim().slice(0, 500);
...
function validateState(state) {
  if (
    ...
    state.schema !== SCHEMA ||
    !state.waves ||
    typeof state.waves !== "object" ||
    Array.isArray(state.waves)
  ) {
  ...
  for (const [waveId, wave] of Object.entries(state.waves)) {
    ...
    if (
      ...
      typeof wave !== "object" ||
      Array.isArray(wave) ||
      !STATES.has(wave.state) ||
      !Array.isArray(wave.leaves) ||
      !wave.leaves.length ||
      !wave.started ||
      typeof wave.started !== "object" ||
      Array.isArray(wave.started) ||
      !wave.returned ||
      typeof wave.returned !== "object" ||
      Array.isArray(wave.returned)
    ) {
    ...
    for (const [leaf, start] of Object.entries(wave.started)) {
      ...
      if (!start || typeof start !== "object" || Array.isArray(start))
        fail("wave " + waveId + " has invalid start for " + leaf);
    ...
    for (const [leaf, returned] of Object.entries(wave.returned)) {
      if (!leaves.has(leaf) || !wave.started[leaf])
        fail("wave " + waveId + " returned unstarted leaf " + leaf);
      if (!returned || typeof returned !== "object" || Array.isArray(returned))
        fail("wave " + waveId + " has invalid return for " + leaf);
    ...
    if (wave.state === "open" && count(wave.returned))
      fail("open wave " + waveId + " contains returns");
    if (wave.state !== "open" && wave.state !== "abandoned" && !allStarted) {
      fail(wave.state + " wave " + waveId + " is missing starts");
    }
    if (wave.state === "complete" && !allReturned)
      fail("complete wave " + waveId + " is missing returns");
    if (wave.state === "sealed" && allReturned)
      fail("sealed wave " + waveId + " should be complete");
    ...
    if (returnTimes.length) {
      if (sealedAt === null)
        fail(wave.state + " wave " + waveId + " contains returns without being sealed");
      if (returnTimes.some((at) => at < sealedAt))
        fail("wave " + waveId + " contains a return before sealing");
    }
    ...
    if (wave.state === "abandoned") {
      if (allReturned)
        fail("abandoned wave " + waveId + " already has every return and must be complete");
...
export function dispatchStatus(root, scope) {
  ...
  let state;
  try {
    state = readState(dispatchStatePath(root, scope));
  } catch (error) {
  ...
  for (const [id, wave] of Object.entries(state.waves).sort(([left], [right]) =>
    ...
    if (wave.state === "abandoned") {
      ...
      result.blocking.push(
        "dispatch:" + id + " open (" + started + "/" + wave.leaves.length + " started)",
      );
    } else {
      result.blocking.push(
        "dispatch:" + id + " sealed (" + returned + "/" + wave.leaves.length + " returned)",
      );
    }
  }
  return result;
    ...
    if (spec.action === "open") {
      ...
      if (spec.action === "abandon") {
        if (current.state === "complete" || current.state === "abandoned") {
          fail(
            "wave " + waveId + " is " + current.state + "; abandon requires an open or sealed wave",
          );
        }
        ...
        if (current.state !== "open")
          fail("wave " + waveId + " is " + current.state + "; start requires an open wave");
        ...
        if (current.state !== "open")
          fail("wave " + waveId + " is " + current.state + "; seal requires an open wave");
        const missing = current.leaves.filter((leaf) => !current.started[leaf]);
        if (missing.length)
          fail("cannot seal " + waveId + ": missing starts for " + missing.join(", "));
        ...
        if (current.state !== "sealed")
          fail("return requires a sealed wave; " + waveId + " is " + current.state);
        if (!current.leaves.includes(leaf)) fail("unknown leaf " + leaf + " in wave " + waveId);
