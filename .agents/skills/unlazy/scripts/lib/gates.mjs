...
export function parseGates(text, options = {}) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (gateMatch) {
      ...
      if (!idMatch)
        errors.push("line " + (index + 1) + ": gate needs an explicit ID followed by a colon");
    ...
    if (INDENTED_ABANDON_RE.test(line)) {
      errors.push(
        "line " + (index + 1) + ": indented ABANDON is not applied; start ABANDON at column 1",
      );
      current = null;
      continue;
    }
    ...
    if (attrMatch) {
      ...
      if (attrs.get(current).has(key)) {
        errors.push(
          "line " + (index + 1) + ": duplicate " + attrMatch[2] + " for gate " + current.id,
        );
      }
    ...
    if (abandonMatch) {
      ...
      if (!id) errors.push("line " + (index + 1) + ": ABANDON needs a gate id and reason");
        ...
        errors.push("line " + (index + 1) + ": ABANDON " + id + " needs a non-blank reason");
      else if (abandoned.has(id))
        errors.push("line " + (index + 1) + ": duplicate ABANDON for " + id);
  ...
  if (options.requireGates !== false && gates.length === 0)
    errors.push("ledger contains zero live gates");
...
export function gateState(gate, abandoned) {
  ...
  const pending =
    gate.evidence === null || gate.evidence === "" || /^pending$/i.test(gate.evidence);
...
export function scopeFiles(root, scope) {
  ...
  try {
    if (statSync(top).isFile()) files.push(top);
  } catch {
    /* absent */
  }
...
export function resolveTarget(options = {}) {
  ...
  if (files.length)
    return { mode: "explicit", scope: null, files: files.map((file) => resolve(root, file)) };
  ...
  if (scopes.length > 1) {
    if (sessionId) {
      const owned = scopes.filter((scope) => {
        try {
          return (
            readFileSync(join(scopeRoot(root, scope), "session"), "utf8").trim() ===
...
function assertSafeStatePath(root, target) {
  ...
  if (existsSync(stateRoot)) {
    ...
    if (info.isSymbolicLink() || !info.isDirectory())
      throw new Error(stateRoot + " must be a real directory, not a link or file");
  }
...
export async function withFileLock(root, target, fn, options = {}) {
  ...
  try {
    writeFileSync(
      fd,
      JSON.stringify({ token, pid: process.pid, target: resolve(target), at: Date.now() }),
    );
    identified = true;
  } catch {
    /* leave for manual cleanup rather than risk deleting a successor */
  }
  try {
    return await fn();
  } finally {
...
export async function claimLeases(root, spec) {
  return withFileLock(root, leaseRegistry(root), () => {
    ...
    if (scopeError || leafError)
      return { ok: false, conflicts: [], error: scopeError || leafError };
    const normalized = [];
    ...
    const file = join(
      lockDirectory(root),
      sha256(spec.scope + "::" + spec.leaf).slice(0, 24) + ".lease",
    );
    writeAtomic(
      file,
      JSON.stringify(
