...
const ACTIVITY_START =
  /^(work(ing)? on|improve|enhance|handle|support|ensure|make sure|try|attempt|look (at|into)|investigate|consider|review|refactor|clean ?up|polish|update|tidy|address|deal with|add support)\b/i;
const findings = [];
  ...
  for (const gate of live) {
    ...
    if (check && FIXED_OUTPUT_COMMAND.test(check)) {
      add(
        ...
        id,
        "tautological-check",
        'CHECK looks like a fixed-output command: "' +
          check +
    ...
    if (expect && WEAK_EXPECT.has(expect.trim().toLowerCase())) {
      add(
        ...
        'EXPECT "' +
          expect +
          '" also appears in failure output; match a line only success can print',
      );
    }
