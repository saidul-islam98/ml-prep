...
function validateSettings(value) {
  ...
  if (value.hooks && value.hooks.Stop !== undefined && !Array.isArray(value.hooks.Stop))
    return "hooks.Stop must be an array";
...
const isOurHandler = (handler) => {
  ...
  const token = (name) =>
    new RegExp("(?:^|\\s)" + name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "(?=\\s|$)").test(value);
  ...
  const packagedStopHook = /(?:^|[\s"'])[^\s"']*[\\/]scripts[\\/]stop-hook\.mjs(?=$|[\s"'])/i.test(
    value,
  );
  ...
  const conventionalV2Path =
    /[\\/]\.claude[\\/]skills[\\/]unlazy[\\/]scripts[\\/]stop-hook\.mjs(?=$|[\s"'])/i.test(value);
  return (
...
if (uninstall) {
  ...
  console.log(
    "Removed unlazy Stop hook (" +
      removed +
      " handler(s)) from " +
      target +
      ". Sibling handlers were preserved.",
  );
  process.exit(0);
}
...
const alreadyCurrent =
  removed === 1 &&
  originalGroups.some((group) =>
    group.hooks.some(
      (handler) =>
        isOurHandler(handler) &&
...
console.log(
  "Installed unlazy Stop hook into " +
    target +
    "\n" +
    "  command: " +
    command +
    "\n" +
    "  scope:   " +
    (scope ? scope : "resolved per session/pipeline") +
    "\n" +
    "  backup:  " +
    (raw ? backup : "not needed for a new file") +
    "\n" +
