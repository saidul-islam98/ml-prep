...
const MAX_BLOCKS = 6;
const safeHostText = (value, max = 500) =>
  String(value)
    .replace(/[\u0000-\u001f\u007f-\u009f\u061c\u200e\u200f\u202a-\u202e\u2066-\u2069]/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
function normalizeHookState(value) {
  ...
  for (const [key, current] of Object.entries(value.sessions)) {
    if (
      ...
      !Number.isInteger(current.blocks) ||
      current.blocks < 0 ||
      typeof current.updatedAt !== "string" ||
      Number.isNaN(Date.parse(current.updatedAt))
    )
      continue;
...
if (target.ambiguous) {
  allow(
    "unlazy: " +
      ...
      ") and none bound to this session; not blocking.",
  );
}
if (target.error && !target.ambiguous)
  allow("unlazy: " + safeHostText(target.error) + "; not blocking.");
...
const handoffMessage = () => {
  ...
  const shown =
    handoffs.slice(0, 5).join(", ") +
    (handoffs.length > 5 ? ", +" + (handoffs.length - 5) + " more" : "");
  return (
    " HANDOFF REQUIRED: " + handoffs.length + " abandoned item(s): " + safeHostText(shown) + "."
  );
};
...
let sessionState;
try {
  sessionState = await withFileLock(
    root,
    statePath,
    () => {
      let state = { schema: 1, sessions: {} };
  ...
  allow(
    "unlazy: could not update the serialized hook state (" +
      safeHostText(error.message) +
      "); not blocking to avoid a trap.",
  );
}
...
if (sessionState.blocks > MAX_BLOCKS) {
  allow(
    "unlazy: releasing after " +
      MAX_BLOCKS +
      " blocks without gate progress" +
      where +
      "; " +
      outstanding.length +
      " item(s) remain (" +
      outstanding.slice(0, 4).join(", ") +
      ")." +
      handoffMessage(),
  );
}
...
const list =
  outstanding.slice(0, 5).join(", ") +
  (outstanding.length > 5 ? ", +" + (outstanding.length - 5) + " more" : "");
console.log(
  JSON.stringify({
    decision: "block",
    reason:
      ...
      ": " +
      outstanding.length +
      " gate/ledger/dispatch item(s) need work: " +
      list +
