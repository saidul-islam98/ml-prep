...
if (actionNames.length > 1)
  failUsage("pipeline actions are mutually exclusive: " + actionNames.join(", "));
...
if (action && (opt.status || opt.reverify || opt.approve))
  failUsage(action + " cannot be combined with a run mode");
...
if (opt.leaf && !opt.claim && !opt.release)
  failUsage("--leaf is only valid with --claim or --release");
...
if (!target.files.length && action !== "--release") {
  failUsage(
    "no gate files found (looked for " +
      UNLAZY_DIR +
      "/<scope>/, then GATES.md and gates/*.md under " +
      root +
      ")",
  );
}
...
function loadLedger(file) {
  ...
  for (const warning of doc.warnings)
    console.error("gate-check: " + file + ": warning: " + warning);
...
if (action === "--claim" || action === "--release") {
  ...
  if (opt.leaf) {
    ...
    if (error) failUsage(error);
    if (stems.length && !stems.includes(opt.leaf))
      failUsage("unknown --leaf " + opt.leaf + " (have: " + stems.join(", ") + ")");
  }
  if (action === "--release") {
    try {
      const count = await releaseLeases(root, { scope, leaf: opt.leaf || null });
      console.log(
        "released " + count + " lease(s) for " + scope + (opt.leaf ? "/" + opt.leaf : ""),
      );
      process.exit(0);
    } catch (error) {
  ...
  let result;
  try {
    result = await claimLeases(root, { scope, leaf, globs: selected.doc.owns });
  } catch (error) {
...
function executableCandidates(name) {
  ...
  return [
    ...
    ...extensions.map((extension) => name + extension.toUpperCase()),
  ];
}
...
function resolveShell(raw) {
  const requested =
    raw ||
    process.env.UNLAZY_SHELL ||
    (process.platform === "win32" ? process.env.ComSpec || "cmd.exe" : "/bin/sh");
  const containsSeparator =
    requested.includes("/") || requested.includes("\\") || isAbsolute(requested);
  const candidates = [];
...
const approvalDir = resolve(
  process.env.UNLAZY_APPROVAL_DIR || join(homedir(), ".unlazy", "approved"),
);
const canonicalRoot = realpathSync(root);
if (!opt.status && pathIsInside(root, approvalDir))
  failUsage("UNLAZY_APPROVAL_DIR must be outside the repository root");
...
function approvalExists(file, gate) {
  ...
  return (
    value &&
    value.file === resolve(file) &&
    value.gate === gate.id &&
    value.signature === signature(file, gate)
  );
}
...
async function recordApproval(file, gate) {
  ...
  try {
    ...
    const value = {
      ...
      file: resolve(file),
      gate: gate.id,
      signature: signature(file, gate),
      oracle: oracle(file, gate),
    ...
    try {
      ...
      /* manual cleanup or a successor owns the lock */
    }
  }
}
...
async function safeRegexMatch(expectation, output) {
  ...
  try {
    return await new Promise((done) => {
      ...
      try {
        worker = new Worker(new URL("./lib/regex-worker.mjs", import.meta.url));
      } catch (error) {
...
function runCheck(task) {
  return new Promise((done) => {
    ...
    const settle = async (exitCode, signal) => {
      ...
      const match =
        timedOut || overflow || spawnError
          ? { matched: false }
          : await safeRegexMatch(task.gate.expectation, output);
      const cleanupSuffix = cleanupDiagnostic ? "; cleanup: " + cleanupDiagnostic : "";
      const error = timedOut
        ? "timed out after " + timeoutSeconds + "s" + cleanupSuffix
        : overflow
          ? "output exceeded " + MAX_OUTPUT_BYTES + " bytes" + cleanupSuffix
          : spawnError
            ? spawnError.message
            : match.error || null;
      done({
  ...
  for (const gate of ledger.doc.gates) {
    ...
    try {
      if (!statSync(cwd).isDirectory())
        failUsage("gate " + qualify(ledger.file, gate.id) + " CWD is not a directory: " + cwd);
    } catch (error) {
      if (error.code === "ENOENT")
        failUsage("gate " + qualify(ledger.file, gate.id) + " CWD does not exist: " + cwd);
    ...
    pending.push({
      ...
      cwd,
      wasMet: state === "met",
      signature: signature(ledger.file, gate),
    });
  }
}
...
for (const task of pending) {
  let approved = false;
  try {
    approved = approvalExists(task.file, task.gate);
  } catch (error) {
    console.error(
      "gate-check: could not validate approval for " +
  ...
  if (!approved) {
    ...
    try {
      ...
      console.log(
        "    APPROVED: " + approvalPath(task.file, task.gate, validatedApprovalDir().path),
      );
    } catch (error) {
      console.error(
        "gate-check: could not record approval for " +
          qualify(task.file, task.gate.id) +
          ": " +
          error.message,
      );
...
function insertOrUpdateEvidence(doc, gate, value) {
  ...
  while (line < doc.lines.length && /^\s+(CHECK|EXPECT|EVIDENCE|CWD):/.test(doc.lines[line]))
    line++;
...
for (const result of results) {
  ...
  try {
    await withFileLock(root, result.file, () => {
      ...
      if (doc.errors.length)
        throw new Error("fresh ledger became invalid: " + doc.errors.join("; "));
      const fresh = doc.gates.find((gate) => gate.id === result.gate.id);
      if (!fresh || signature(result.file, fresh) !== result.signature) {
        staleResults.set(
          resultKey(result.file, result.gate.id),
          qualify(result.file, result.gate.id),
        );
        console.log(
          "  STALE " +
            qualify(result.file, result.gate.id) +
            ": CHECK/EXPECT/CWD/shell signature changed; result not written",
        );
        return;
      }
      if (result.ok) {
...
for (const result of results) {
  if (opt.reverify && result.wasMet && !staleResults.has(resultKey(result.file, result.gate.id)))
    reverified++;
}
  ...
  for (const gate of ledger.doc.gates) {
    ...
    if (state === "abandoned") {
      ...
      if (opt.status) {
        console.log(
          "  UNMET " +
            qualify(ledger.file, gate.id) +
            " (" +
            (state === "unmet" ? "unchecked" : "checked but EVIDENCE pending") +
            "): " +
            gate.title,
        );
      }
...
for (const task of unverifiedMet) {
  ...
  if (state === "met" || state === undefined)
    extraUnmet.set(key, qualify(task.file, task.gate.id) + " (reverify not run)");
}
for (const [key, label] of staleResults) {
  const state = finalStates.get(key);
  if (state === "met" || state === undefined)
    extraUnmet.set(key, label + " (stale result discarded)");
}
...
if (approvalInfrastructureFailures) {
  console.error(
    "gate-check: infrastructure failure prevented " +
      approvalInfrastructureFailures +
      " approval(s)",
  );
  process.exit(2);
...
if (totalAbandoned) {
  console.log(
    "HANDOFF REQUIRED: " +
      totalAbandoned +
      " abandoned (met: " +
      Math.max(0, totalMet - extraUnmet.size) +
      (effectiveUnmet ? ", unmet: " + effectiveUnmet : "") +
      verifyNote +
      ")" +
      where,
  );
...
if (effectiveUnmet) {
  console.log(
    "UNMET: " +
      ...
      " (met: " +
      Math.max(0, totalMet - extraUnmet.size) +
      (totalAbandoned ? ", abandoned: " + totalAbandoned : "") +
      verifyNote +
      ")" +
      where,
  );
