...
while (args.length) {
  ...
  if (!["--root", "--scope", "--wave", "--leaf", "--handle", "--reason"].includes(option))
...
if (command === "open") {
  ...
  if (options.handle !== null || options.reason !== null)
  ...
  if (options.leaves.length || options.handle !== null)
...
const summary = (wave, id) => {
  ...
  if (wave.state === "complete")
    return "COMPLETE " + id + " (" + returned + "/" + wave.leaves.length + " returned)";
...
try {
  ...
  if (command === "open")
    ...
    console.log("SEALED " + options.wave + " (" + started + "/" + wave.leaves.length + " started)");
  else if (command === "abandon") console.log(summary(wave, options.wave));
  else if (wave.state === "complete")
    console.log(
      "COMPLETE " + options.wave + " (" + returned + "/" + wave.leaves.length + " returned)",
    );
  else
    console.log(
      "RETURNED " +
